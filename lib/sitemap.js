...
function getEntryConfig(file, siteConfig) {
    ...
    const mappingsForFile =
        find(siteConfig.mappings, function(item) {
...
function notIsHTTPURL(text) {
    return !/https?:\/\//gi.test(text);
}

function getImagesUrl(entry, siteConfig) {
    const reImageMatch = /<img\s+(data-)?src="(.+?)\\*"/gi;
    const reSourceMatch = /"(.+?)\\*"/gi;
    ...
    const URLs = images.map(image => {
        let currentURL = image.match(reSourceMatch)[0].replace(/^\"|\"$/gi, "");
...
function generateImagesMap(entry, siteConfig) {
    ...
    if (imagesURLS.length) {
        XMLImageList = imagesURLS
            .map(imageURL => createImageSitemap(imageURL))
            .join("");
    }
...
function processEntry(entry, siteConfig) {
    ...
    const loc = entry.getLoc
        ? entry.getLoc(siteConfig.siteUrl, entry.loc, entry)
        : entry.loc;
    returnArr.push(
        siteConfig.spacing +
            siteConfig.spacing +
            wrapTag("loc", loc) +
            (siteConfig.images ? generateImagesMap(entry, siteConfig) : "")
    );
